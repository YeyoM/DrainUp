...
class ParserAnalyzer:
    ...
    def load_csv_data(self, csv_files):
        ...
        for parser, filepath in csv_files.items():
            # Read CSV: skip first row (column numbers), use second row as header
            df = pd.read_csv(filepath, skiprows=[0], index_col=0, header=0)
            ...
            if not self.datasets:
                # Get dataset names (exclude 'Average' column if present)
    ...
    def load_parsing_times(self, time_files):
        ...
        # Debug: show what datasets we have in parsing times vs CSV
        print("\nDataset name comparison:")
        print(f"  CSV datasets: {self.datasets[:3]}...")
        for parser in self.parsing_times:
            time_datasets = list(self.parsing_times[parser].keys())[:3]
            print(f"  {parser} time datasets: {time_datasets}...")
    ...
    def generate_summary_statistics(self):
        ...
        if not time_means.isna().all():
            speed_winner = time_means.idxmin()
            print(f"  {'Speed':12s}: {speed_winner} ({time_means[speed_winner]:.4f}s)")
        else:
            print(f"  {'Speed':12s}: Unable to determine (parsing times not loaded)")
    
    def plot_metric_comparison(self, output_dir='plots_full'):
    ...
    def plot_per_dataset_performance(self, output_dir='plots_full'):
    ...
    def plot_speed_vs_accuracy(self, output_dir='plots_full'):
    ...
    def plot_radar_chart(self, output_dir='plots_full'):
    ...
    def plot_winners_per_dataset(self, output_dir='plots_full'):
    ...
    def run_full_analysis(self):
        ...
        print("="*80)
        print("\nCheck the 'plots_full/' directory for visualizations")
...
def main():
    ...
    csv_files = {
        'Drain': '../../result/Drain_full_complex=0_frequent=0.csv',
        'UniParser': '../../result/UniParser_full_complex=0_frequent=0.csv',
        'DrainUP': '../../result/DrainUP_full_complex=0_frequent=0.csv',
    }
    
    time_files = {
        'Drain': '../../result/result_Drain_full/parsing_times.json',
        'UniParser': '../../result/result_UniParser_full/infer_time_full.txt',
        'DrainUP': '../../result/result_DrainUP_full/parsing_times.json',
    }
